...
[Ссылка на очередь проверки](https://github.com/alex65536/bulatov-lectures-2/projects/2)

## Скачать PDF

PDF регулярно собирается на сервере, из последнего коммита в ветке `master`.
Для скачивания можно использовать
[эту ссылку](https://github.com/alex65536/bulatov-lectures-2/releases/download/latest/bulatov.pdf).

Между коммитом и сборкой новой PDFки по нему может пройти около пяти минут.
В случае, если сборка свлится, PDFка не обновится.
...
- Новые понятия следует выделять `\emph{курсивом}`. `\textit` использовать не стоит.
