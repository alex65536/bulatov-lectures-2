...
\end{equation}
Полагая, в соответствии с \eqref{lec25:3}, для $\forall z \in D \implies \exists! f(z) \in \C$.
 Полагая 
$\begin{cases} 
...
\end{cases}
 \]
 Для изображения общей ФКП потребуется 4-мерное пространство. Для изучения
 свойств  общих ФКП используются 2 двумерных пространства. Первое соотвествует
 комплексной плоскости \textcircled{z}, на которой используются $x$ и $y$, второе соотвествует
...
\end{equation}
	...
	\end{cases}$
		\begin{enumerate}
			...
			\]
			 ...
			 \end{cases}
			\end{equation}
			В таком случае имеем параллельный перенос на вектор, соответствующий радиус-вектору числа $b$.
			
			\includegraphics[height=0.4\textwidth]{lec25_5.png}
		\end{enumerate}
		В общем случае для $a \ne 0$ общую линейную ФКП можно записать в следующем виде:
		\[
	           w = az + b = a\left(z + \frac{b}{a}\right) = a(z + z_0).
		\]
		Применим линейную ФКП сначала к вектору $w_1 = z + z_0$ --- параллельному переносу $z$  на радиус-вектор числа $z_0 = \frac{b}{a}$.
		 Получим $w = aw_1$, где:
		 \[
		...
		Образ любой линейной ФКП получается с помощью линейного переноса, растяжения/сжатия и поворота.
		\end{enumerate}
	...
	\end{cases}
	\end{equation}
	Если $c = 0, d \ne 0$, то  $w \overset{\eqref{lec25:7}}{=} \frac{az + b}{d} = a_0{z} + b_0, \ 
	\begin{cases} 
 	 a_0  = \frac{a}{d} \in \C\\
  	 b_0  = \frac{b}{d} \in \C
	\end{cases}
	 $ --- переходим к линейной ФКП. В этом случае $\Delta = \begin{vmatrix}a&b\\ c&d\end{vmatrix} = ad - bc = 0$.
	 
	Пусть  $\Delta \ne 0, c \ne 0$. Тогда имеем:
	\begin{equation}
	\label{lec25:8}
	w \overset{\eqref{lec25:7}}{=} \frac{a}{c} - \frac{\Delta}{c^{2}(z + \frac{d}{c})}
	\end{equation}
