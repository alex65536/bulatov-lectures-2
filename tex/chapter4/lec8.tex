...
0}^{b}f(x)dx = \underset{\scriptsize{
\begin{gathered}
\eps_1 \to 0\\
\eps_2 \to 0                                                                   
                                                                               
                                                               \end{gathered}}}
\lim\left(\int\limits_{a}^{c - \eps_1}f(x)dx + \int\limits_{c + 
\eps_2}^{b}f(x)dx\right)
\end{equation}

Если в $\eqref{lec8:5}$ каждое слагаемое сходится, то рассматриваемый НИ-2 
также считается сходящимся, в противном случае расходится.
...
\begin{enumerate}
 \item[1.] Для сходимости интеграла от неотрицательной функции $f(x)$ при 
 ...
 \item[2.] Предельный признак сравнения для сходимости НИ-2 вида 
 $\eqref{lec8:3}$:
 ...
 \begin{itemize}
  \item[a)] $0 < p < +\infty \implies \displaystyle\int\limits_{a}^{b}f(x)dx$ 
 ...
 \begin{itemize}
 \item[] Если имеет НИ-2 вида $\eqref{lec8:3}$, то в случае, когда $f(x) 
...
\leq b$, в кототрых $f(x)$ не ограничена, причем в остальных точках 
