...
\begin{exmp}
 ...
 ~--- расходится, откуда по признаку сравнения расходится и $\displaystyle\int\limits_{1}^{+\infty} \dfrac{|\sin x|dx}{x^{\alpha}}$.
\end{exmp}
...
Если:
\begin{itemize}
\item[a)] $H(x) = \displaystyle\int\limits_{a}^{x}h(t)dt$ ограничена $\forall x \in 
