...
\begin{thm}[интегрирование по частям в НИ]
	...
	где $ b = +\infty $, либо подынтегральная функция неограничена в окрестности $ b $. Если
...
Когда НИ расходятся в обычном смысле, они могут сходится в смысле \emph{главного
...
чтобы $ c_k \in ]b_{k - 1}; b_k[ $ для $\forall k = \overline{q, n}$, и других $ c_i $ 
на $ ]b_{k - 1}; b_k[ $ нет.
\item
...
\underbrace{\int\limits_\frac{3}{2}^\frac{7}{2}\dfrac{xdx}{x^2 - 4x + 3}}_{I_2} +
...
I_2$, т.~е. НИ сходится в смысле главного значения. Кроме того, $ \vp I_3 = I_3 \in \R $ ~--- сходится. Т.~е., хотя исходный НИ расходится, он будет сходиться в смысле главного значения.
\end{exmp}
\begin{exercise}
	Вычислить в случае $ a < c < b : \vp \displaystyle\int\limits_a^b \dfrac{dx}{x - c} $.
\end{exercise}
