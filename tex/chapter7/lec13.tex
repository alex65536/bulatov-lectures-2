...
\]
$\Gamma^{(k)}(a)$ непрерывна $\forall a > 0$, т.~е. гамма-функция бесконечное число раз непрерывно дифференцируема.
...
0 + 1 = 1
\end{gather*}
В таком случае, т.~к. $\Gamma(1) = \Gamma(2)$, то по теореме Ролля $\exists a_0 \in ]1, 2[$ такое, что $\Gamma'(a_0) = 0$, т.~е. $a_0$~--- 
стационарная 
...
\begin{center}
\includegraphics[scale = 0.8]{../../img/lec13_1} 
\end{center}

Интегрирование по частям дает:
\[
\Gamma(a) = \int\limits_0^{+\infty} e^{-x} d\left(\dfrac{x^a}{a}\right) =
 \dfrac{e^{-x}x^a}{a} \Big|^{+\infty}_0 - \int\limits_0^{+\infty} \frac{x^a}{a} 
 d(e^{-x}) = 
...
\end{equation}
При $a \to 0$ имеем
\[\Gamma(a) \sim \frac{\Gamma(1)}{a} = \dfrac{1}{a},\] т.~е. $a=0$~--- вертикальная асимптота.
