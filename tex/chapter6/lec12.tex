...
Нетрудно показать, что \eqref{lec12:9} сходится равномерно для
$a \in [0; +\infty[$ в силу признака Абеля равномерной сходимости НИЗОП-1.
Воспользовавшись следствием из обобщенного интеграла Эйлера-Пуассона,
получим:
\[\frac1{\sqrt t} = \frac2{\sqrt\pi} \ioinf 
...
= \frac\pi{2b} \in \R.\] При этом $e^{ab} \appr{a\to \infty} \infty,\ b>0$. 
...
b}^{\beta b} \frac{f(t)}t dt
= \left(\,\int\limits_{\alpha a}^{\alpha b} \dfrac{f(t)}t dt
+ \int\limits_{\alpha b}^{\beta a} \dfrac{f(t)}t dt\right)
- \left(\,\int\limits_{\alpha b}^{\beta a} \frac{f(t)}t dt
+ \int\limits_{\beta a}^{\beta b} \frac{f(t)}t dt\right) =\]
\[= \int\limits_{\alpha a}^{\alpha b} \frac{f(t)}t 
...
Если $f \in C([0; +\infty))$ и $\exists f(+\infty) \in \R$, то тогда для 
...
\end{gathered}
\right] \implies \]
\[ \implies \Phi(a, b) = (f(0) - f(+\infty))\cdot \ln\left(\frac 
ba\right) \]
\end{proof}
