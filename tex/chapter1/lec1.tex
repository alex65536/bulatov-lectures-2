...
\begin{proof}
	Рассмотрим $g(t)=\sum\limits_{n=1}^{\infty}u_n'(t),
	 t\in\left[a,b\right]$. В силу условий теоремы и теоремы
	  ...
	  \end{gather*}
	  Применяя теорему Барроу о дифференцировании интеграла с переменным верхним пределом от непрерывной функции, имеем:
	  \begin{equation*}
	  	\exists f'(x)=(C)'+(\int_{x_0}^xg(t)dt)'_x=0+g(t)\bigg|_{t=x}=g(x)
	  	\Leftrightarrow\ref*{eq:19}
	  \end{equation*} 
\end{proof}	
...
\begin{crl*}
	...
	\begin{thm}[о почленном дифференцировании ФП]
		 ...
		 $
		 то в случае, когда $ f_n\left(x\right)  \overset{[a; b]}{\underset{n\to \infty}\rightrightarrows},$ имеем:
		 \begin{equation*}
