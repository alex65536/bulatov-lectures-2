...
\begin{proof}
 ...
 то из $\eqref{lec1:5} \iff \underset{x \in E}{\sup}|f_n(x) - f(x)| \leq \eps,\ 
 \forall n \geq \nu_\eps$, т.~е.
...
\underset{n \to \infty}{\nrightarrow}0$, то 
$f_n(x) \overset{E}{\nrightrightarrows} f(x)$ (не сходится равномерно либо 
сходится поточечно, либо расходится).
 \end{itemize}	
\end{rem}	
...
В данном случае множество сходимости рассматриваемой последовательности $D = ]-1;1]$. 
...
\underbrace{|f_n - f(x)|}_{\leq \eps} \leq \\ \leq 2\eps$, т.~е. 
...
не зависит от $x \in E$ имеем $\eqref{lec1:8} 
\underset{m \to \infty}{\implies}$ существует предельная функция 
$\eqref{lec1:6}$, для которой $|f_n(x) - f(x)| = 
\underset{m \to \infty}{\lim} |f_n(x) - f_m(x)| \leq \\ \leq \eps \ 
