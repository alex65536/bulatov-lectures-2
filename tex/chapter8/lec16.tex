...
\begin{thm}[о существовании конечного положительного периода]
	...
	\begin{erem}
	Здесь не показано, почему при отсутствии наименьшего положительного периода
	инфимум множества всех периодов должен быть равен нулю. Это следует из того
	факта, что если $T_1$ и $T_2$ периоды и $T_1 \ne T_2$, то тогда
	$|T_1 - T_2|$ также будет являться периодом.
	\end{erem}
...
\end{enumerate}
	...
	$f_0(x) \equiv 0 \quad \forall x \in \left[a, b\right]$ имеем
...
\item В-четвертых,
\[\forall k, m \in \N,\ k \neq m, \implies \scal{\sin kx, \sin mx} =
