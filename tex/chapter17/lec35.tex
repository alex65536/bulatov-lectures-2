...
\section{Основная теорема о вычетах}
	...
	\begin{defn}
		Точка $z_0$ называется \emph{устранимой особой точкой (УОТ)} функции $f(z)$, если
		\[\exists \lim_{z\to z_0}f(z)\in \mathbb{C}.\]
	\end{defn}

	\begin{defn}
		Точка $z_0$ называется \emph{полюсом $k$-го порядка} функции $f(z)$ если все пределы вида
	...
	\begin{defn}
		Точка $z_0$ называется \emph{существенной особой точкой (СОТ)} функции $f(z)$, если
	...
	\begin{examples}
		\begin{enumerate}
			...
			\[
				f(z) = \dfrac{1- \left(1-\dfrac{z}{1!}+\dfrac{z^2}{2!}-\ldots\right)}{z^2} = \frac{1}{z} - \frac{1}{2} + \frac{z}{6}- \ldots
			\]
			Из разложения следует, что $\underset{0}{\res}f(z) = c_{-1} = 1.$ Также в силу \eqref{35.3}
			\[
	...
	\begin{example}
		...
		\[
			\underset{0}{\res}f(z) = \dfrac{1}{1!}\lim_{z\to 0}\left(z^2\left(\dfrac{1-e^{-z}}{z^2}\right)\right)' = 
			\lim_{z\to 0} (1 - e^{-z})' = 1.
		\]
	\end{example}
