...
\begin{thm}
Теорема.О перестановке порядка интегрирования  в повторном интеграле у одного из которых имеется бесконечный предел.
\end{thm}
\\
Пусть функция $f(x,y)$ непрерывна на $[a;+\infty)\times [c;d]$, если $F(y) = \int\limits_a^{+\infty}f(x,y)dx \overset{y \in [c;d]}{\rightrightarrows}$ , то   $\exists \int\limits_c^d F(y)dy =\int\limits_c^d\left(\int\limits_a^{+\infty}f(x,y)dx\right)dy=\int\limits_a^{+\infty}\left(\int\limits_c^d f(x,y)dy\right)dx $
\end{document}