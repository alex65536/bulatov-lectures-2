...
\text{[последов-ти Гейне]} \implies f(x, y_n) 
...
\begin{thm}[о предельном переходе в СИЗОП]
	Пусть:
	\begin{enumerate}
		\item При каждом $\fix y \in D$ функция $ f(x, y) $ непрерывна
		для $\forall x \in [a, b]$
		\item $f(x, y) 
		\stackrel{[a, b]}{\underset{y \to y_0}{\rightrightarrows}}
		g(x)$, где $y_0$ ~--- предельная точка для $D$		
	\end{enumerate}	
	Тогда, во-первых, $g(x) \in C([a, b])$, и, во-вторых, для СИЗОП
	\eqref{lec5:1} получаем:
	\begin{equation}
	\label{lec6:6}
...
\begin{proof}
	...
	Рассмотрим $(y_n) \in D$ ~--- последовательность Гейне предельной точки $y_0$.
...
\begin{proof}
	...
	\stackrel{[a, b]}{\underset{\Delta y \to 0}{\rightrightarrows}} f_y'(x, y_0).
	\end{equation}
...
\begin{rem}[о дифференцировании СИЗОП с переменными пределами интегрирования]
	...
	а их значения не выходят за соответствующий отрезок $[a^*;b^*]$, и
	$f(x, y), f'_y(x, y) \in C([a^*; b^*] \times [c; d])$, то тогда имеем 
	следующее обобщение формулы Лагранжа:
