...
\label{lec6:2}
	...
	% нормально под стрелкой сделать, хз как
\end{equation}
...
\begin{exmp}
	...
	$ \displaystyle\int\limits_0^1 f_0(x) dx = \displaystyle\int\limits_0^1 f(x) dx $.
	Теперь рассмотрим $ F(y) = \displaystyle\int\limits_0^1 \dfrac{x^y - 1}{\ln x} dx $
	...
	F'(y) = \displaystyle\int\limits_0^1 x^y dx = \left.\dfrac{x^{y + 1}}{y + 1}\right|_{x=0}^{x=1} = \dfrac{1}{y + 1}
	.\] Из полученного уравнения следует, что после
	неопределённого интеграла получаем \[F'(y) = \dfrac{1}{y + 1} \implies F(y) = \int \dfrac{dy}{y + 1} = 
	\ln(1 + y) + C.\] Далее в силу теоремы о предельном переходе в СИЗОП получаем, что
