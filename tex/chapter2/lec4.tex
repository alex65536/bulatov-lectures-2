...
\begin{proof}
	...
	\sum u_n(x) \rightrightarrows$ по признаку Вейерштрасса, т.~к. 
	$|u_n(x)| \le r^n|a_n|$, а мажоранта сходится в силу того, что 
	$0 < r < R$.
\end{proof}
...
\begin{rems}
	...
	\begin{enumerate}
		...
		и тот же $R$ и $I$, их множества сходимости могут быть разными.
...
\begin{thm}[О почленном интегрировании степенного ряда]
	...
	\begin{equation} \label{lec4:11}
		\exists \int\limits_{\alpha}^{\beta} 
		\sum\limits_{n = 0}^{\infty}
		a_n(x - x_0)^n dx = \sum\limits_{n = 0}^{\infty} a_n
		\int\limits_{\alpha}^{\beta} (x - x_0)^n dx
	\end{equation}
