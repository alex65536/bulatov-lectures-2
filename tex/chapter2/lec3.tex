...
\begin{rem}
	...
	результатов и определения точной верхней грани получим, что СтР \ref{lec3_2:1}-\ref{lec3_2:2}
	будет сходиться при $\abs{x-x_0}\leq R$ и расходиться при $\abs{x-x_0}>R$.
	
	При этом в случае, когда $R=+\infty \implies x=R=\left[-\infty;+\infty\right]$.
	Если же $R=0$, то $X=\{x_0\}$.	
\end{rem}	
В общем  случае неотрицательная величина $R$ (конечная или бесконечная) 
называется \textit{радиусом сходимости}  \ref{lec3_2:1}-\ref{lec3_2:2}, а 
промежуток $I=\left]x_0-R;x_0+R\right[$ называется \textit{интервалом сходимости} 
