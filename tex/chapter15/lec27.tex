...
\section{Основная теорема о вычетах}
	...
	является либо устранимой особой точкой,
	либо полюсом, либо существенно особой точкой, и во всех случаях изолированна, 
	...
	Коэффициент $c_{-1}$ в этом разложении называется вычетом $f(z)$ в точке 
	$z_0$ и обозначается 
	...
	соответствующий вычет. Из \eqref{35.1} следует, что есил $z_0$ --- правильная 
	...
	\begin{proof}
		...
		Обычным образом показывается, что $I = \ointctrclockwise\limits_{l} f(z)dz,$ 
		также как и при доказательстве теоремы Коши. 
		В силу \eqref{35.1} имеем $\ointctrclockwise\limits_{l} f(z)dz = 2\pi i 
		c_{-1} = 2 \pi i \sum_{k=1}^{m}\underset{z_k}{res} f(z) \implies 
		\eqref{35.2}.$
	\end{proof}	
...
\section{Ряд Лорана и вычеты БУТ(бесконечно удаленной точки)}
	...
	c_p\neq 0
	 ...
	 \begin{itemize}
	 	\item[1)] В разложении $f(z)$ в окрестности $z=\infty$ нет главной части, и 
	 	...
	 	Из \ref{35.7} следует, что если $z=\infty$ --- нуль кратности $m$, то 
	 	...
	 	\item[3)] 
	 	БУТ является СОТ. В этом случае как правило считается через ряд Лорана. 
